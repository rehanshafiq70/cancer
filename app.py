...
:root {
    --bg:#03080f; --bg2:#061120; --card:rgba(6,18,38,0.85);
    --border:rgba(0,212,255,0.18); --teal:#00d4ff; --green:#00ff88;
    --red:#ff3355; --yellow:#ffcc00; --text:#cce8ff; --muted:#3a6080;
    --font1:'Orbitron',monospace; --font2:'Inter',sans-serif;
}
html,body,[data-testid="stAppViewContainer"],.main {
    background:var(--bg)!important; color:var(--text); font-family:var(--font2);
}
...
.stButton>button {
    ...
    color:#00d4ff!important; border:1px solid rgba(0,212,255,0.5)!important;
    border-radius:8px!important; font-family:var(--font1)!important;
    font-size:0.72rem!important; font-weight:600!important;
    letter-spacing:0.08em!important; padding:0.5rem 1.2rem!important;
...
[data-testid="stMetricValue"] {
    color:var(--teal)!important; font-family:var(--font1)!important; font-size:1.7rem!important;
}
...
.stTabs [data-baseweb="tab"] {
    font-family:var(--font1)!important; font-size:0.65rem!important;
...
def init_state():
    D = {
        "page": "Home", "model": None, "model_loaded": False,
        "model_status": "not_loaded", "scan_history": [],
        "total_scans": 0, "malignant_cnt": 0, "benign_cnt": 0,
    }
    for k, v in D.items():
...
# ── MODEL LOADER ──────────────────────────────────────────────────
GDRIVE_ID  = "18A3F0XdVmuqnoWTD_UvsAKw2iIvLghXf"
    ...
    if not os.path.exists(MODEL_FILE):
        ...
        try:
            ...
            gdown.download(
                ...
                MODEL_FILE, quiet=False, fuzzy=True
            )
        ...
        if not downloaded:
            try:
                ...
                if token:
                    resp = session.get(URL, params={"id": GDRIVE_ID, "confirm": token}, stream=True)
...
def validate_image(img):
    if img.width < 64 or img.height < 64:
        return False, "Image resolution too low (min 64x64 px)."
...
def predict(model, img, status):
    tensor = preprocess(img)
    if model is not None and status == "loaded":
        raw = model.predict(tensor, verbose=0)
        mal = float(raw[0][0]) if raw.shape[-1] == 1 else float(raw[0][1])
    else:
    ...
    return {"label": label, "conf": conf,
            "mal_pct": round(mal*100,1), "ben_pct": round(ben*100,1),
            "risk": risk, "demo": status != "loaded"}

# ── REPORT ────────────────────────────────────────────────────────
def make_report(pat, res):
    ...
    mode = "REAL CNN" if not res["demo"] else "DEMO MODE"
    return f"""
╔══════════════════════════════════════════════════════════╗
...
Always confirm with a licensed physician.
...
def render_sidebar():
    with st.sidebar:
        ...
        dc = "#00ff88" if is_live else "#ffcc00"
        dl = "CNN Online" if is_live else "Demo Mode"
        st.markdown(f"""
        <div style="margin:0 12px 14px;padding:7px 12px;border-radius:8px;
          ...
          <span style="width:7px;height:7px;border-radius:50%;background:{dc};
                       box-shadow:0 0 8px {dc};display:inline-block;"></span>
          <span style="font-size:0.72rem;color:{dc};font-family:'Orbitron',monospace;">{dl}</span>
        </div>""", unsafe_allow_html=True)

        st.markdown("<hr>", unsafe_allow_html=True)
        nav = {
            "🏠  Home": "Home", "🔬  AI Scan": "Scan",
            "📊  Dashboard": "Dashboard",
...
def page_home():
    ...
    for col, val, lbl in [
        (d1,"10,015","Total Images"),(d2,"6,705","Benign Cases"),
        (d3,"3,310","Malignant Cases"),(d4,"Kaggle","Source"),
    ]:
        with col:
            st.markdown(f"""
...
def page_scan():
    ...
    if not st.session_state.model_loaded:
        ...
        if status == "loaded":
            st.success("✅ CNN model loaded — Real predictions enabled!")
        else:
            st.warning(f"⚠️ Demo Mode: {status}")
    ...
    with col_ctrl:
        ...
        if not ok:
            ...
            <div style="background:rgba(255,51,85,0.08);border:1px solid rgba(255,51,85,0.3);
                        border-radius:10px;padding:1rem;color:#ff3355;">
    ...
    if run:
        ...
        for pct, txt in [(15,"Preprocessing image…"),(35,"Extracting feature maps…"),
                         (60,"Running convolutional layers…"),(80,"Computing output…"),
        ...
        if res["conf"] < 60:
            ...
            <div style="background:rgba(255,204,0,0.07);border:1px solid rgba(255,204,0,0.3);
                        border-radius:10px;padding:0.8rem;color:#ffcc00;font-size:0.8rem;
                        margin-bottom:1rem;">
              ⚠️ Low confidence ({res['conf']}%) — upload a higher quality image.
            </div>""", unsafe_allow_html=True)
        ...
        <div style="background:rgba(6,18,38,0.9);border:1.5px solid {lc}44;
          ...
          <div style="height:5px;background:rgba(255,255,255,0.05);
            ...
            <div style="height:100%;width:{res['conf']}%;
                        background:linear-gradient(90deg,{lc}66,{lc});border-radius:5px;">
            </div>
          </div>
          <div style="display:flex;gap:10px;">
            <div style="flex:1;background:rgba(255,255,255,0.03);border-radius:8px;
              ...
              <div style="font-size:0.65rem;color:#3a6080;text-transform:uppercase;">Risk</div>
              <div style="font-family:'Orbitron',monospace;font-size:0.9rem;
                          font-weight:700;color:{rc};">{res['risk']}</div>
            </div>
            <div style="flex:1;background:rgba(255,255,255,0.03);border-radius:8px;
                        padding:8px 12px;border:1px solid rgba(255,255,255,0.05);">
              <div style="font-size:0.65rem;color:#3a6080;text-transform:uppercase;">Malignant</div>
              <div style="font-family:'Orbitron',monospace;font-size:0.9rem;
                          font-weight:700;color:#ff3355;">{res['mal_pct']}%</div>
            </div>
            <div style="flex:1;background:rgba(255,255,255,0.03);border-radius:8px;
                        padding:8px 12px;border:1px solid rgba(255,255,255,0.05);">
              <div style="font-size:0.65rem;color:#3a6080;text-transform:uppercase;">Benign</div>
              <div style="font-family:'Orbitron',monospace;font-size:0.9rem;
        ...
        with ch1:
            fig_bar = go.Figure(go.Bar(
                x=["Benign","Malignant"], y=[res["ben_pct"],res["mal_pct"]],
            ...
            fig_bar.update_layout(
                ...
                xaxis=dict(color="#3a6080"), height=280,
            )
        ...
        with ch2:
            fig_g = go.Figure(go.Indicator(
                ...
                gauge={
                    ...
                    "bar":{"color":lc}, "bgcolor":"rgba(0,0,0,0)",
                    "steps":[
            ...
            fig_g.update_layout(
                paper_bgcolor="rgba(0,0,0,0)", font_color="#cce8ff", height=280)
        ...
        if res["label"] == "Malignant":
            ...
            <div style="background:rgba(255,51,85,0.06);border:1px solid rgba(255,51,85,0.3);
              ...
              <div style="font-family:'Orbitron',monospace;font-size:0.75rem;color:#ff3355;
                ...
                🚨 MALIGNANT — URGENT ACTION REQUIRED
              </div>
        ...
        patient = {
            "name": p_name or "Unknown", "age": p_age,
            "gender": p_gender,
        ...
        <div style="margin-top:1rem;padding:0.8rem 1rem;
          ...
          AI result assists clinicians only. Confirm with a licensed dermatologist.
        </div>""", unsafe_allow_html=True)
...
def page_dashboard():
    ...
    c3.metric("⚠️ Malignant",       mal)
    c4.metric("✅ Benign",           ben)

    if not history:
        st.info("No scan data yet. Go to AI Scan to get started.")
        return

    cl, cr = st.columns(2)
    with cl:
        fig = px.pie(names=["Benign","Malignant"], values=[ben,mal],
    ...
    with cr:
        fig2 = go.Figure(go.Bar(
            ...
            marker_color=["#ff3355" if h["diagnosis"]=="Malignant" else "#00ff88"
                          for h in history[-20:]],
        ))
        fig2.update_layout(title="Confidence per Scan",
                           paper_bgcolor="rgba(0,0,0,0)",
                           plot_bgcolor="rgba(0,0,0,0)",
                           font_color="#cce8ff", title_font_family="Orbitron",
                           yaxis=dict(range=[0,110],color="#3a6080"))
...
def page_guide():
    ...
    unsafe_allow_html=True)
    steps = [
        ("01","👤","Patient Details","Fill patient name, age, gender, and ID before scanning."),
        ("02","🖼️","Upload Image","Upload a clear JPG or PNG dermoscopic image (min 64x64 px)."),
        ("03","⚡","Execute Scan","Click EXECUTE DEEP SCAN — real CNN processes the image."),
        ("04","🧬","View Result","See Malignant/Benign result, confidence %, and risk level."),
        ("05","📄","Download Report","Click Medical Report to download clinical report (.txt)."),
        ("06","📊","Dashboard","Analytics, charts, and historical scan statistics."),
        ("07","📥","Export Data","Export all records as CSV from the Registry page."),
    ]
...
def render_footer():
    ...
    <div style="text-align:center;padding:1rem 0 0.5rem;
      ...
      <div>Rehan Shafique | SkinScan AI v12.0</div>
    </div>""", unsafe_allow_html=True)
